...
Three claim-backed constraint fields ship: `price_per_person` (`value_number`), `cuisine` (`value_text`), `district` (`located_in`, `value_text`). Alongside them, three entity-level filters are hard constraints and are enforced, not accepted and ignored: `entity_types` against `Entity.entity_type`, `entity_subtypes` against `Entity.subtype`, and `user_state` against `EntityUserState.state`. Each narrows candidate generation in SQL and is re-checked per entity in `_evaluate_entity`, which emits `entity_type_mismatch`, `entity_subtype_mismatch` or a user-state rejection. `entity_types` is validated against a closed set; `entity_subtypes` is not, so a caller may pass a subtype that matches nothing.

The supported `user_state` filters are exactly the three product intent states — `want_to_go`, `want_to_try`, `want_to_learn` — shared with Resurface as `knowledge/resurface.py:INTENT_STATES`. `visited` is intentionally unsupported: finishing something is a history, a different feature, and there is no write path for it, so parsing 去过 into a constraint the executor cannot satisfy would produce a plan that claims to filter on history and silently returns everything. A plan carrying it is rejected with `unsupported_user_state` rather than accepted and dropped.

An `entity_types`-only plan is not by itself structured — `is_structured` requires a location, a claim constraint or a user state — so such a question routes to ordinary hybrid retrieval instead of reaching the executor. The filter is a narrowing constraint, not a trigger.
...
Restaurants are `entity_type="place"`; there is no `restaurant` entity type. `subtype` is enforced when a plan carries it, but the deterministic parser deliberately never proposes `restaurant` — production extraction does not reliably write `Entity.subtype`, so a parser that manufactured the filter would reject every restaurant in the corpus. The distinction is the point: the executor honours the field for a caller who knows the data supports it, and the parser does not invent one. Building a subtype classifier to close the gap is out of scope.

**Consequence**: the order of operations is itself the contract, and each step is load-bearing.

A `source_ids` allow-list restricts the claims the executor may read *before* qualification, not the citations afterwards, so an entity supported only by an excluded source does not qualify at all. `None` (no restriction) and `[]` (restricted to nothing) stay distinct. Qualification then evaluates the *full* qualifying set — bounded by `MAX_CANDIDATES = 500`, never truncated to `plan.limit` — because stopping at the limit would hand similarity an arbitrary prefix of the qualifying set to order. FTS and vector search run next, over the surviving sources only, so ranking never sees a rejected candidate. `plan.limit` is applied last, to the ranked list.

`semantic_requirements` are soft signals throughout. They contribute to the ranking text and to a rank-based per-source bonus, and they appear nowhere in the executor; a soft requirement can reorder an answer but can never exclude anyone from it. Similarity therefore cannot resurrect a candidate a hard constraint rejected, and this is structural rather than a tuning choice: membership is fixed by `execute()` before any similarity call, ranking is a sort over that list, and a rejected entity is in a different list entirely. There is no weight, threshold or floor on the path that could be adjusted to re-admit it.

The canonical path is integrated, not a side entrance: `ConversationManager.ask()` classifies scope, resolves follow-ups, parses and persists the plan, and routes to `retrieve_structured` when the scope uses the collection and the plan is structured — so the guarantees above hold for the real Ask surface rather than only for a test harness.

**Evidence**: `retrieval/query_plan.py` (`validate_plan`, `NUMERIC_OPERATORS`, `CLAIM_FIELDS`, `USER_STATES`); `retrieval/query_parser.py` (`parse_query`); `retrieval/structured.py` (`StructuredExecutor._evaluate_entity`, `_eligible_claims_for`, `MAX_CANDIDATES`); `retrieval/retriever.py` (`retrieve_structured`, `_rank_structured_matches`, `_semantic_scores`); `retrieval/vocabulary.py`; `conversation/conversation_manager.py` (`ask`); `conversation/answer_generator.py` (`_structured_answer`, `_no_result_reasons`); `tests/test_structured_retrieval.py`.

---

### DEC-021 — A condition V1 cannot express is refused, not approximated

**Decision**: a claim is eligible only if it has at least one `ClaimEvidence` whose `EvidenceUnit.source_id` equals the claim's own `source_id`. A structured query stating a condition V1 cannot express is *refused* — the turn returns an honest "V1 cannot apply this condition" and runs no retrieval at all — rather than degraded to a search over the raw text. A model proposal is validated as one unit: a single unsupported field invalidates the whole proposal. A merged entity is returned as its survivor, never under its own deprecated name.

**Rationale**: four failures that all took the same shape — answering a *nearby* question and presenting it as the user's.

Evidence had two holes. A claim with zero `ClaimEvidence` still satisfied a constraint, so an entity could be listed as supported structured knowledge with nothing behind it; and a claim could be cited using evidence from a *different* source, fabricating provenance across the `Claim → ClaimEvidence → EvidenceUnit → Source` spine. `GroundingValidator` already hard-rejects that second shape at write time, so retrieval now enforces the identical rule at read time. One rule closes both, in both mirrored eligibility implementations plus the citation map.

Parsing had two more. Cuisine and district are matched by bare substring scan, so `不是日料的旺角餐厅` produced `cuisine = 日料` and returned exactly what the user excluded. `人均100以下和200以上` collapsed into `>= 100` — a predicate matching neither stated bound and admitting the whole band excluded twice. Refusal rather than `QueryPlanError` is the load-bearing detail: a plan error degrades to unstructured retrieval over the original string, and a text search for `不是日料` matches `日料`, so "failing" through that path reproduces the defect one layer down. And a model answering `{"district": "九龙城", "cuisine": "日料"}` had its unresolvable district dropped before validation, executing the cuisine alone — a question about 九龙城 answered with 旺角 restaurants. Raw values now reach `validate_plan`, which already rejects out-of-vocabulary ones, so the proposal succeeds or fails whole.

**Consequence**: refusal is visible, narrow, and not a general negation language. `parse_query` records `diagnostics["refused"]`, `ConversationManager.ask` skips retrieval on it, and `_no_result_reasons` returns that message ahead of every retrieval-level reason — those all describe a search that ran, and a refused query never ran one. The detector covers exactly two shapes: a negation cue immediately preceding a district or cuisine alias, and two numbers each carrying their own comparison marker. It deliberately does *not* cover `人均大概80`, which states no comparison: that stays unsupported-and-silent, emitting no numeric constraint rather than inventing a threshold, and is asserted as a boundary so the detector cannot grow into it.

Two smaller corrections fell out. `RetrievalResult.is_empty()` ignored `structured.matches` while `source_ids` on the same dataclass counted them, so a user-state-only plan — which qualifies entities with no claims at all — reported a false no-result at `ask()` while the executor had found the entity. And `_structured_answer` defaulted `has_evidence=True`, announcing evidence for an answer with an empty citation list; it is now derived from the citations.

**Evidence**: `knowledge/eligibility.py` (rule 6, both implementations); `conversation/citation_builder.py` (`_claim_evidence_map`); `retrieval/query_parser.py` (`refused_condition`, `_propose_plan_with_model`); `retrieval/structured.py` (`ConstraintSupport.merge_with`, merge redirection in `execute`); `retrieval/retriever.py` (`is_empty`); `conversation/conversation_manager.py` (`ask`, `_next_state`); `conversation/answer_generator.py` (`_no_result_reasons`, `_structured_answer`); `tests/test_structured_retrieval_evidence.py`, `tests/test_structured_retrieval_merge.py`, `tests/test_structured_retrieval_refusal.py`, `tests/test_structured_retrieval_state_only.py`.
...
**Query planning now exists, over three fields.** The old `retrieval/query_planner.py` was deleted rather than wired (regex place-name patterns, its own `TODO: use an NER model`, exported and instantiated nowhere; `Settings.query_planner_model` and `Settings.enable_query_enrichment` went with it). DEC-020 replaced it with a validated `QueryPlan` over `district`, `cuisine` and `price_per_person`, plus the `entity_types`, `entity_subtypes` and `user_state` filters described there. What remains unimplemented from the `QueryPlanInput` sketch in `RETRIEVAL.md` 5: ranges (`50-100`), time constraints, `sort_preferences`, relations, and `city`, which is accepted and carried but never executed. There is no conjunction on a single field, so two constraints on one field are rejected rather than combined. A question outside the supported fields degrades to ordinary hybrid retrieval, which is correct behaviour but is not structured retrieval.

Within the shipped filters, four honest limits. Negation is not *executed*: `present=False` is a valid plan field that nothing in V1 proposes, and a negated district or cuisine is refused outright rather than parsed (DEC-021). `entity_subtypes` values are not validated against a closed set, unlike `entity_types`. The `MAX_CANDIDATES` truncation is silent — no diagnostic distinguishes a complete scan from a capped one. And the source allow-list governs qualification through the claims the executor reads, so for the degenerate shape of a *user-state-only* plan — no location, no claim constraint — there are no claims to restrict and membership comes from `EntityUserState` alone. That is the intended reading of DEC-018, not a gap: the user's own intent has a different lifetime from a creator's claim, so it does not expire when its source stops qualifying. Such a plan therefore returns the entity with nothing cited beneath it, and `has_evidence` is false.

**Structured ranking is source-level, and candidate selection is order-dependent.** Two known imprecisions, recorded rather than fixed. First, `MAX_CANDIDATES = 500` bounds the candidate set *before* qualification, and the pre-cap selection follows database/insertion order — so with more than 500 candidates an entity can disappear before ever reaching the deterministic fallback and ranking stage the contract above describes. Second, ranking scores a *source*, not an entity-evidence pair: one source discussing several qualifying entities gives all of them the same score, and the excerpt rendered beneath a chosen entity can come from the part of that source discussing a different one. Both are precision limits on ordering and illustration, not on membership — no entity is admitted or rejected by either — which is why neither blocks V1.

**`text_requirements` is validated and carried but never read.** `query_plan.py` describes it as a ranking signal by contract, and no ranking code consults it: only `semantic_requirements` reaches `_ranking_query` and `_semantic_scores`. `intent`, `notes` and `conversation_entity_refs` are likewise persisted without being executed — follow-up resolution happens earlier and separately, in `_resolve_followup`, by appending the previous turn's entity names to the query text. These are inert rather than wrong, but the plan is a wider record than the executor is a consumer of. Follow-up resolution is still `conversation/conversation_manager.py:_resolve_followup` reading the previous turn's entities from conversation state.
