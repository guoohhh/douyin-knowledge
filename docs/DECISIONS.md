...
Within the shipped filters, four honest limits. Negation is not parsed: `present=False` is a valid, executed plan field, but nothing in V1 proposes one, so 不想去 keeps a query's other constraints and contributes no user-state filter. `entity_subtypes` values are not validated against a closed set, unlike `entity_types`. The `MAX_CANDIDATES` truncation is silent — no diagnostic distinguishes a complete scan from a capped one. And the source allow-list governs qualification through the claims the executor reads, so for the degenerate shape of a *user-state-only* plan — no location, no claim constraint — there are no claims to restrict and membership comes from `EntityUserState` alone; every allow-list test uses a claim-bearing plan.
