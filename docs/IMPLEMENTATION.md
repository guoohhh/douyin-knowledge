...
`FileCaptureProvider` or the Douyin sidecar 5.1+ REST adapter supplies source metadata. Sync deduplicates by `(platform, external_id)`, retains current collection memberships and records source snapshots, policy decisions, and sync results, and evaluates user rules. Eligible sources enter a SQLite job queue. The worker creates a versioned ProcessingRun, retains caption/transcript EvidenceUnits, optionally transcribes short-caption video with a configured audio API, extracts claims, validates each claim's quoted text against an EvidenceUnit, resolves entities by normalized exact identity, creates KnowledgeItem, updates Wiki revisions and support links, and updates FTS/vector projections from current run claim evidence. FTS keeps complete referenced evidence text; embedding input is capped at 4,000 characters. Retrieval combines direct source/entity matching, FTS, and vector scores. Answers expose source, claim, and evidence IDs. Explicit “want to go/try/learn” personal states produce a source-backed resurfacing list. A configured AI endpoint can generate a prose answer from the retrieved claims.

## Decisions differing from the original design

| Original design | Implemented choice | Reason and trade-off |
| --- | --- | --- |
| ~50 physical tables in five migrations | Focused relational tables plus FTS5 in six migrations | A compact slice could be verified end to end. Current collection membership, historical source snapshots, and policy decisions are retained; dedicated collection event history, topic links, detailed job events, and conversation threads are deferred. A small Wiki quality ledger tracks deterministic lint findings and fixes. |
...
- Questions without a personal-collection cue use general scope. Local retrieval ranks claims within matching sources and selects relevant predicates when a clear question cue is present.
- Successful sidecar sync treats its listing as complete: absent sidecar-owned saves become `removed`, leave current retrieval/Wiki, and can be restored later. Source and processing history remain stored.
